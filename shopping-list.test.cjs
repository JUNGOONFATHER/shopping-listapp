/**
 * 쇼핑 리스트 앱 자동 테스트
 * 실행: node shopping-list.test.cjs
 */
...
(async () => {
  ...
  assert(emptyVisible, '초기 상태에서 "아이템을 추가해 보세요!" 메시지가 보임');
  ...
  assert(statsText.trim() === '', '초기 통계 텍스트가 비어있음');
  ...
  const inputAfterAdd = await page.locator('#itemInput').inputValue();
  assert(inputAfterAdd === '', '추가 후 입력창이 비워짐');
  ...
  assert(firstLiClass && firstLiClass.includes('checked'), '체크된 항목에 "checked" 클래스 추가됨');

  const strikethrough = await page.locator('#list li .item-text').first().evaluate(
  ...
  items = await page.locator('#list li').count();
  assert(items === 0, '완료 항목 일괄 삭제 후 리스트가 비어있음');
  ...
  items = await page.locator('#list li').count();
  assert(items === 2, '새로고침 후 아이템 2개 유지됨 (로컬 스토리지)');
